...
async function getKnowledgeBase(workspaceId: string, category?: string, source?: string) {
    let query = supabase
        .from('knowledge_base')
        .select('*')
    ...
    if (error) throw new Error(`Failed to fetch KB: ${error.message}`);
    return data;
}

async function searchKnowledge(workspaceId: string, query: string, limit = 10) {
    const { data, error } = await supabase
        .from('knowledge_base')
        .select('*')
        .eq('workspace_id', workspaceId)
...
async function addKnowledge(workspaceId: string, entry: {
    category: string;
    title: string;
    content: string;
    source_type: 'sam' | 'vera' | 'manual' | 'api';
    tags?: string[];
...
async function getICPData(workspaceId: string) {
    const { data, error } = await supabase
...
async function getCRMContacts(workspaceId: string, limit = 50) {
    const { data, error } = await supabase
        .from('sam_accounts')
...
async function getCampaigns(workspaceId: string, status?: string) {
    let query = supabase
        .from('sam_campaigns')
...
async function getContentQueue(workspaceId: string, status?: string) {
    let query = supabase
        .from('content_queue')
        .select('*')
        .eq('workspace_id', workspaceId);
...
async function getApprovedContent(workspaceId: string, platform?: string, limit = 20) {
    let query = supabase
        .from('content_queue')
        .select('*')
        .eq('workspace_id', workspaceId)
    ...
    if (error) throw new Error(`Failed to fetch approved content: ${error.message}`);
    return data;
}
...
async function syncContentToSAM(workspaceId: string, contentId: string, contentType: string) {
    ...
    if (error) throw new Error(`Failed to sync to SAM: ${error.message}`);
    return data;
}

async function getLeadsForRetargeting(workspaceId: string, criteria?: {
    status?: string;
        ...
        try {
            ...
            switch (name) {
                ...
                case 'get_workspace_context':
                    result = await getWorkspaceContext(args?.workspace_id as string);
                    break;
                case 'get_knowledge_base':
                    result = await getKnowledgeBase(
                ...
                case 'search_knowledge':
                    ...
                    break;
                case 'add_knowledge':
                    result = await addKnowledge(args?.workspace_id as string, {
                        category: args?.category as string,
                        title: args?.title as string,
                        content: args?.content as string,
                        source_type: args?.source_type as 'sam' | 'vera' | 'manual' | 'api',
                        tags: args?.tags as string[]
                    });
                    break;
                case 'get_icp_data':
                    result = await getICPData(args?.workspace_id as string);
                    break;
                case 'get_crm_contacts':
                    result = await getCRMContacts(args?.workspace_id as string, args?.limit as number);
                    break;
                case 'get_campaigns':
                    result = await getCampaigns(args?.workspace_id as string, args?.status as string);
                    break;
                case 'get_content_queue':
                    result = await getContentQueue(args?.workspace_id as string, args?.status as string);
                    break;
                case 'get_approved_content':
                    result = await getApprovedContent(
                        args?.workspace_id as string,
                ...
                case 'get_leads_for_retargeting':
                    result = await getLeadsForRetargeting(args?.workspace_id as string, {
                        status: args?.status as string,
                        minEngagement: args?.min_engagement as number
                    });
                    break;
                default:
