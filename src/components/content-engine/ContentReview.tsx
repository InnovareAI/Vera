...
<head>
    ...
    <title>${campaignData?.campaign || 'Content Export'} - LinkedIn Posts</title>
    <style>
...
<body>
    <div class="container">
        <div class="header">
            <h1>${campaignData?.campaign || 'LinkedIn Content Series'}</h1>
            <p>Source: ${campaignData?.source || 'Generated Content'} • ${approvedPosts.length} posts ready for publishing</p>
        </div>
        ...
        <div class="post">
            ...
            ${post.imageUrl ? `<img class="post-image" src="${window.location.origin}${post.imageUrl}" alt="${post.theme}">` : ''}
            ...
            <div class="cta">
                ...
                <div class="cta-text">${campaignData?.cta || ''} → ${campaignData?.ctaUrl || ''}</div>
            </div>
        </div>
        `).join('')}
        <div class="footer">
            Generated by VERA • ${new Date().toLocaleDateString()} • Ready for LinkedIn publishing
        </div>
    </div>
</body>
</html>`
        ...
        a.href = url
        a.download = `${(campaignData?.campaign || 'content-export').replace(/\s+/g, '-').toLowerCase()}-${new Date().toISOString().split('T')[0]}.html`
