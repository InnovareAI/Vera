        ...
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <Link href="/commenting">
              <button className="px-4 py-2 text-sm font-medium text-neutral-400 hover:text-neutral-100 hover:bg-white/5 rounded-lg transition-all">
                Monitors
              </button>
            </Link>
            ...
            <span className="px-4 py-2 text-sm font-medium text-neutral-100 bg-white/10 rounded-lg">
              Analytics
            </span>
          </div>
        </div>
      ...
      <main className="max-w-7xl mx-auto px-6 py-12 space-y-12">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            ...
            <h2 className="text-4xl font-semibold tracking-tight text-neutral-100 mb-2">Commenting Analytics</h2>
          ...
          {/* Range selector */}
          <div className="flex items-center gap-1 bg-neutral-900/60 border border-neutral-800 rounded-xl p-1">
            {(['7d', '30d', '90d'] as Range[]).map((r) => (
              <button
                ...
                className={`px-4 py-2 rounded-lg text-sm font-bold transition-all ${
                  range === r
                    ? 'bg-violet-600 text-neutral-100 shadow-lg shadow-violet-600/20'
        ...
        {loading ? (
          ...
          <>
              ...
              {[
                ...
                >
                  ...
                  <div className="flex items-end gap-3">
                    <span className={`font-semibold text-neutral-100 ${stat.isText ? 'text-2xl truncate max-w-[180px]' : 'text-5xl'}`}>
                      {stat.value}
                    </span>
            ...
            {data.daily_trend.length > 0 && (
              <div className="bg-neutral-900/40 border border-neutral-800 rounded-xl p-8">
                <div className="flex items-center justify-between mb-8">
                  <div>
                    <h3 className="text-xl font-semibold text-neutral-100">Daily Comment Volume</h3>
                    <p className="text-sm text-neutral-500 mt-1">Comments posted per day</p>
                  </div>
                ...
                <div className="flex items-end gap-1 h-48">
                  {data.daily_trend.map((day, i) => {
                    ...
                    return (
                      ...
                      >
                        <span className="text-xs text-neutral-500 font-bold opacity-0 group-hover:opacity-100 transition-opacity">
                          {day.count}
                        </span>
            ...
            {data.top_monitors.length > 0 && (
              <div className="bg-neutral-900/40 border border-neutral-800 rounded-xl p-8">
                <div className="flex items-center justify-between mb-6">
                  <div>
                    <h3 className="text-xl font-semibold text-neutral-100">Top Monitors</h3>
                    <p className="text-sm text-neutral-500 mt-1">Monitors ranked by comment volume</p>
                  </div>
                  <span className="text-neutral-500 text-sm font-bold">{data.top_monitors.length} monitors</span>
                </div>
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="border-b border-neutral-800/50">
                        <th className="text-left text-neutral-500 font-bold uppercase tracking-widest text-xs py-4 px-6">Monitor</th>
                        <th className="text-right text-neutral-500 font-bold uppercase tracking-widest text-xs py-4 px-6">Comments</th>
                        <th className="text-right text-neutral-500 font-bold uppercase tracking-widest text-xs py-4 px-6">Avg Confidence</th>
                      </tr>
                    </thead>
                    <tbody>
                        ...
                        >
                          <td className="py-4 px-6">
                            <div className="flex items-center gap-3">
                              ...
                              <span className="font-bold text-neutral-100">{monitor.monitor_name}</span>
                            </div>
                          </td>
                          <td className="py-4 px-6 text-right font-mono text-sm text-neutral-300">
            ...
            <div className="bg-neutral-900/40 border border-neutral-800 rounded-xl p-8">
              <div className="flex items-center justify-between mb-6">
                <div>
                  <h3 className="text-xl font-semibold text-neutral-100">Recent Posted Comments</h3>
                  <p className="text-sm text-neutral-500 mt-1">Latest AI-generated comments that were posted</p>
                </div>
                <span className="text-neutral-500 text-sm font-bold">Last 20</span>
              </div>
              ...
              {data.recent_comments.length === 0 ? (
                <div className="text-center py-16">
                  ...
                  <p className="text-neutral-500 font-medium">No comments posted yet</p>
                  <p className="text-neutral-600 text-sm mt-1">Comments will appear here once the agent starts posting.</p>
                </div>
              ) : (
                <div className="space-y-3">
                  {data.recent_comments.map((comment) => (
                    <div
                      key={comment.id}
                      className="bg-neutral-800/30 border border-neutral-800/50 rounded-2xl p-5 hover:bg-neutral-800/50 transition-colors"
                    >
                      <div className="flex items-start justify-between gap-4">
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-2">
                            <span className="text-xs font-bold text-neutral-400 uppercase tracking-widest">Replying to</span>
                            <span className="text-sm font-bold text-neutral-100">{comment.post_author}</span>
                            <span className="text-neutral-700 text-xs">|</span>
                            <span className="text-xs text-neutral-500">{formatDateTime(comment.posted_at)}</span>
                          </div>
