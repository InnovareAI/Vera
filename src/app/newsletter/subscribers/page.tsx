...
function statusBadge(status: Subscriber['status']) {
  switch (status) {
    ...
    case 'unsubscribed':
      return <Badge className="bg-gray-600/20 text-gray-400 border border-gray-600/30">Unsubscribed</Badge>
    case 'bounced':
    ...
    <div className="min-h-screen bg-gray-950 text-white selection:bg-violet-500/30">
      ...
      <header className="relative z-50 border-b border-gray-800/50 bg-gray-950/50 backdrop-blur-xl">
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-white font-medium text-sm">Subscribers</h1>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <Dialog open={importDialogOpen} onOpenChange={(v) => { setImportDialogOpen(v); if (!v) { setCsvData(''); setImportResult(null) } }}>
              ...
              <DialogContent className="bg-gray-900 border-gray-700 text-white max-w-lg">
                <DialogHeader>
                  <DialogTitle className="text-white">Import Subscribers from CSV</DialogTitle>
                ...
                <div className="space-y-4 mt-4">
                  <div>
                    <Label className="text-gray-400 text-xs font-bold uppercase tracking-widest mb-2 block">Upload CSV File</Label>
                    <input
                      type="file"
                      accept=".csv,text/csv"
                      onChange={handleFileUpload}
                      className="w-full text-sm text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-violet-500/20 file:text-violet-400 hover:file:bg-violet-500/30 cursor-pointer"
                    />
                  </div>
                  <div className="text-center text-gray-600 text-xs">or paste CSV below</div>
                  <Textarea
                    value={csvData}
                    onChange={(e) => setCsvData(e.target.value)}
                    placeholder="email,first_name,last_name&#10;john@example.com,John,Doe&#10;jane@example.com,Jane,Smith"
                    className="bg-gray-800 border-gray-700 text-white placeholder:text-gray-600 min-h-[120px] font-mono text-xs"
                  />
                  {importResult && (
            ...
            <Dialog open={addDialogOpen} onOpenChange={setAddDialogOpen}>
              ...
              <DialogContent className="bg-gray-900 border-gray-700 text-white">
                <DialogHeader>
                  ...
                  <DialogDescription className="text-gray-400">Add a new subscriber to your newsletter.</DialogDescription>
                </DialogHeader>
                <div className="space-y-4 mt-4">
                  <div>
                    <Label className="text-gray-400 text-xs font-bold uppercase tracking-widest mb-1 block">Email *</Label>
                    <Input
                      type="email"
                      value={addForm.email}
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <Label className="text-gray-400 text-xs font-bold uppercase tracking-widest mb-1 block">First Name</Label>
                      <Input
                    ...
                    <div>
                      <Label className="text-gray-400 text-xs font-bold uppercase tracking-widest mb-1 block">Last Name</Label>
                      <Input
                  ...
                  <div>
                    <Label className="text-gray-400 text-xs font-bold uppercase tracking-widest mb-1 block">Tags (comma-separated)</Label>
                    <Input
                      value={addForm.tags}
                  ...
                  <div className="flex gap-3 justify-end">
                    <Button variant="ghost" onClick={() => setAddDialogOpen(false)} className="text-gray-400 hover:text-white">Cancel</Button>
      ...
      <main className="max-w-7xl mx-auto px-6 py-12 space-y-8">
        ...
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-gray-700 text-xs">/</span>
          <span className="text-violet-400 text-xs font-bold uppercase tracking-widest">Subscribers</span>
        </div>

        <div>
          <h2 className="text-4xl font-black tracking-tight text-white mb-2">Subscriber Management</h2>
          <p className="text-lg text-gray-400">View, manage, and grow your newsletter audience.</p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
          {[
            ...
            <div key={stat.label} className="bg-gray-900/40 border border-gray-800 p-6 rounded-3xl">
              <p className="text-gray-500 font-bold uppercase tracking-widest text-xs mb-1">{stat.label}</p>
              <span className="text-5xl font-black text-white">{loading ? <Skeleton className="h-12 w-16" /> : stat.value}</span>
            </div>
          ))}
        </div>

        {/* Filters */}
        <div className="bg-gray-900/20 border border-gray-800/50 rounded-[2.5rem] p-8">
          <div className="flex flex-wrap items-center gap-4 mb-6">
            ...
            <Select value={statusFilter} onValueChange={(v) => setStatusFilter(v as typeof statusFilter)}>
              ...
              <SelectContent className="bg-gray-900 border-gray-700">
                ...
                <SelectItem value="unsubscribed" className="text-white hover:bg-gray-800">Unsubscribed</SelectItem>
                <SelectItem value="bounced" className="text-white hover:bg-gray-800">Bounced</SelectItem>
              </SelectContent>
            </Select>
            {allTags.length > 0 && (
              <Select value={tagFilter} onValueChange={setTagFilter}>
                ...
                <SelectContent className="bg-gray-900 border-gray-700">
                  <SelectItem value="all" className="text-white hover:bg-gray-800">All Tags</SelectItem>
                  {allTags.map((tag) => (
                    <SelectItem key={tag} value={tag} className="text-white hover:bg-gray-800">{tag}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            )}
              ...
              <Table>
                <TableHeader>
                  <TableRow className="border-gray-800 hover:bg-transparent">
                    ...
                    <TableHead className="text-gray-500 font-bold uppercase tracking-widest text-xs">Email</TableHead>
                    <TableHead className="text-gray-500 font-bold uppercase tracking-widest text-xs">First Name</TableHead>
                    <TableHead className="text-gray-500 font-bold uppercase tracking-widest text-xs">Last Name</TableHead>
                    <TableHead className="text-gray-500 font-bold uppercase tracking-widest text-xs">Tags</TableHead>
                    <TableHead className="text-gray-500 font-bold uppercase tracking-widest text-xs">Status</TableHead>
                    <TableHead className="text-gray-500 font-bold uppercase tracking-widest text-xs">Subscribed</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {filteredSubscribers.map((sub) => (
                    <TableRow key={sub.id} className="border-gray-800/50 hover:bg-gray-900/40 transition-colors">
                      <TableCell>
                        <Checkbox
                      ...
                      <TableCell className="text-gray-400">{sub.first_name || '--'}</TableCell>
                      <TableCell className="text-gray-400">{sub.last_name || '--'}</TableCell>
                      <TableCell>
                      ...
                      <TableCell className="text-gray-500 text-sm">{formatDate(sub.subscribed_at)}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
