...
export default function NewProjectPage() {
  ...
  if (!user || !currentWorkspace) {
    return (
      <div className="min-h-screen bg-neutral-950 flex items-center justify-center">
        <div className="text-center">
          <h2 className="text-2xl font-semibold text-neutral-100 mb-2">Authentication Required</h2>
          <p className="text-neutral-500 mb-6">Please log in and select a workspace to continue.</p>
          <Link href="/login" className="px-8 py-3 bg-violet-500 text-neutral-100 font-bold rounded-xl">
            Go to Login
          </Link>
        </div>
      </div>
  ...
  const renderTagInput = (
    ...
    <div>
      <label className="block text-sm font-bold text-neutral-400 mb-2">{label}</label>
      ...
      <input
        ...
        className="w-full bg-neutral-800/50 border border-neutral-700 rounded-xl px-4 py-3 text-neutral-100 placeholder-gray-600 focus:outline-none focus:border-violet-500/50 transition-colors"
      />
        ...
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          <div className="flex items-center gap-8">
            ...
            <nav className="hidden md:flex items-center gap-1">
              ...
              <Link href="/projects" className="px-4 py-2 text-sm font-medium text-neutral-100 bg-white/10 rounded-lg transition-all">
                Projects
              </Link>
      ...
      <main className="max-w-3xl mx-auto px-6 py-12 space-y-10">
        ...
        <div>
          <div className="flex items-center gap-2 mb-4">
            <Link href="/projects" className="text-neutral-500 hover:text-neutral-300 text-xs font-bold uppercase tracking-widest transition-colors">
              Projects
            </Link>
          ...
          <h1 className="text-4xl font-semibold tracking-tight text-neutral-100 mb-2">Create a New Project</h1>
          <p className="text-lg text-neutral-400">Set up your brand, products, audience, and channels.</p>
        </div>
            ...
            >
              <div
                className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-semibold transition-all ${
                  i === step
                    ? 'bg-violet-500 text-neutral-100 shadow-lg shadow-violet-500/20'
                    : i < step
              ...
              <span
                className={`text-xs font-bold uppercase tracking-widest hidden sm:block ${
                  i === step ? 'text-neutral-100' : i < step ? 'text-violet-400' : 'text-neutral-600'
                }`}
              >
                {s}
              </span>
              {i < STEPS.length - 1 && (
                <div className={`w-8 h-px hidden sm:block ${i < step ? 'bg-violet-500/40' : 'bg-neutral-800'}`} />
              )}
            </button>
          ))}
        </div>
        ...
        {/* Step Content */}
        <div className="bg-neutral-900/40 border border-neutral-800 rounded-xl p-8 space-y-6">
          {/* Step 1: Brand Info */}
          {step === 0 && (
            <>
              <h2 className="text-2xl font-semibold text-neutral-100">Brand Info</h2>
              <div>
                <label className="block text-sm font-bold text-neutral-400 mb-2">Project Name *</label>
                <input
                  type="text"
                  value={name}
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  className="w-full bg-neutral-800/50 border border-neutral-700 rounded-xl px-4 py-3 text-neutral-100 placeholder-gray-600 focus:outline-none focus:border-violet-500/50 transition-colors resize-none"
                />
              </div>
              <div>
                <label className="block text-sm font-bold text-neutral-400 mb-2">Website URL</label>
                <div className="flex gap-3">
                  <input
                    ...
                    className="flex-1 bg-neutral-800/50 border border-neutral-700 rounded-xl px-4 py-3 text-neutral-100 placeholder-gray-600 focus:outline-none focus:border-violet-500/50 transition-colors"
                  />
                  <button
                    type="button"
                    onClick={fetchBrandInfo}
                    disabled={extracting || !websiteUrl.trim()}
                    className="px-5 py-3 bg-violet-500 text-neutral-100 font-bold rounded-xl hover:opacity-90 transition-all disabled:opacity-40 disabled:cursor-not-allowed flex items-center gap-2 whitespace-nowrap text-sm"
                  >
                    {extracting ? (
                      <>
              ...
              <div>
                <label className="block text-sm font-bold text-neutral-400 mb-2">Industry</label>
                <input
                  type="text"
                  value={industry}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-bold text-neutral-400 mb-2">Primary Color</label>
                  <div className="flex items-center gap-3">
                    ...
                    <input
                      ...
                      value={primaryColor}
                      onChange={(e) => setPrimaryColor(e.target.value)}
                      className="flex-1 bg-neutral-800/50 border border-neutral-700 rounded-xl px-4 py-3 text-neutral-100 font-mono text-sm focus:outline-none focus:border-violet-500/50 transition-colors"
                    />
                  </div>
                </div>
                <div>
                  <label className="block text-sm font-bold text-neutral-400 mb-2">Secondary Color</label>
                  <div className="flex items-center gap-3">
                    <input
                      type="color"
          ...
          {step === 1 && (
            <>
              <div className="flex items-center justify-between">
                <h2 className="text-2xl font-semibold text-neutral-100">Products & Services</h2>
                <button
                  onClick={addProduct}
                  className="px-4 py-2 text-sm font-bold text-violet-400 hover:text-neutral-100 bg-violet-500/10 hover:bg-violet-500/20 rounded-lg border border-violet-500/20 transition-all"
                >
                  + Add Product
                </button>
              </div>
              <div className="space-y-6">
                {products.map((product, i) => (
                  <div key={i} className="bg-neutral-800/30 border border-neutral-700/50 rounded-xl p-6 space-y-4 relative">
                    {products.length > 1 && (
                      <button
                        onClick={() => removeProduct(i)}
                        className="absolute top-4 right-4 p-1.5 text-neutral-600 hover:text-red-400 transition-colors"
                      >
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div className="text-xs font-bold text-neutral-500 uppercase tracking-widest">Product {i + 1}</div>
                    <div>
                    ...
                    <div>
                      <label className="block text-sm font-bold text-neutral-400 mb-2">URL (optional)</label>
                      <input
                        type="url"
                        value={product.url}
          ...
          {step === 2 && (
            <>
              <h2 className="text-2xl font-semibold text-neutral-100">Ideal Customer Profile</h2>
              ...
              <div>
                <label className="block text-sm font-bold text-neutral-400 mb-2">Company Size</label>
                <select
                  value={companySize}
                  onChange={(e) => setCompanySize(e.target.value)}
                  className="w-full bg-neutral-800/50 border border-neutral-700 rounded-xl px-4 py-3 text-neutral-100 focus:outline-none focus:border-violet-500/50 transition-colors"
                >
          ...
          {step === 3 && (
            <>
              <h2 className="text-2xl font-semibold text-neutral-100">Tone of Voice</h2>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-bold text-neutral-400 mb-2">Style</label>
                  <select
                    value={style}
                    onChange={(e) => setStyle(e.target.value)}
                    className="w-full bg-neutral-800/50 border border-neutral-700 rounded-xl px-4 py-3 text-neutral-100 focus:outline-none focus:border-violet-500/50 transition-colors capitalize"
                  >
                    {STYLE_OPTIONS.map((s) => (
                ...
                <div>
                  <label className="block text-sm font-bold text-neutral-400 mb-2">Formality</label>
                  <select
                    value={formality}
              ...
              <div>
                <label className="block text-sm font-bold text-neutral-400 mb-2">Dos (one per line)</label>
                <textarea
                  value={dosText}
              ...
              <div>
                <label className="block text-sm font-bold text-neutral-400 mb-2">Don&apos;ts (one per line)</label>
                <textarea
                  value={dontsText}
          ...
          {step === 4 && (
            <>
              <h2 className="text-2xl font-semibold text-neutral-100">Enable Platforms</h2>
              <p className="text-neutral-500 text-sm">Select the channels this project will publish to.</p>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                {PLATFORMS.map((platform) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className={`w-12 h-12 rounded-xl mx-auto flex items-center justify-center text-lg font-semibold mb-3 ${
                          active
                      ...
                      <span className={`text-sm font-bold ${active ? 'text-neutral-100' : 'text-neutral-500'}`}>
                        {platform.label}
                      </span>
                    </button>
                  )
        ...
        <div className="flex items-center justify-between">
          <button
            ...
            className="px-6 py-3 text-neutral-400 hover:text-neutral-100 font-bold rounded-xl hover:bg-white/5 transition-all"
          >
            {step === 0 ? 'Cancel' : 'Back'}
          </button>
          ...
          {step < STEPS.length - 1 ? (
            <button
              ...
              disabled={!canProceed()}
              className="px-8 py-3 bg-violet-500 text-neutral-100 font-bold rounded-xl hover:opacity-90 transition-opacity disabled:opacity-40 disabled:cursor-not-allowed"
            >
              Next
            </button>
          ) : (
            <button
              ...
              className="px-8 py-3 bg-violet-500 text-neutral-100 font-bold rounded-xl hover:opacity-90 transition-opacity disabled:opacity-40 disabled:cursor-not-allowed flex items-center gap-2"
            >
              {submitting ? (
                <>
                  <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin"></div>
