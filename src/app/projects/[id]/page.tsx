...
export default function ContentBenchPage() {
  ...
  // Fetch project context for AI generation
  ...
  return (
    <div className="space-y-6">
      {/* Create Content Button / Panel */}
      {!showCreator ? (
        <button
          onClick={() => setShowCreator(true)}
          className="w-full p-4 rounded-2xl border-2 border-dashed border-gray-700 hover:border-violet-500/50 bg-gray-900/30 hover:bg-violet-900/10 transition-all group flex items-center justify-center gap-3"
        >
          <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-violet-500 to-fuchsia-500 flex items-center justify-center text-white text-xl font-bold shadow-lg shadow-violet-500/20 group-hover:scale-110 transition-transform">
            +
          </div>
        ...
        <div className="rounded-2xl border border-gray-800 bg-gray-900 overflow-hidden">
          {/* Creator Header */}
          <div className="p-5 border-b border-gray-800 flex items-center justify-between bg-gradient-to-r from-violet-900/20 to-fuchsia-900/20">
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-violet-500 to-fuchsia-500 flex items-center justify-center">
                <span className="text-white text-sm font-bold">+</span>
              </div>
              <h3 className="text-white font-bold">Create New Content</h3>
            </div>
            <button
              onClick={() => {
                setShowCreator(false)
              ...
              className="text-gray-500 hover:text-white w-8 h-8 rounded-lg hover:bg-white/10 flex items-center justify-center transition-all"
            >
              &times;
            </button>
          </div>
          ...
          <div className="p-5 space-y-5">
            ...
            <div>
              ...
              <textarea
                ...
                rows={3}
                className="w-full px-4 py-3 rounded-xl bg-gray-800 border border-gray-700 focus:border-violet-500/50 focus:outline-none focus:ring-2 focus:ring-violet-500/20 text-white placeholder-gray-600 text-sm resize-none transition-all"
              />
            </div>
            ...
            <div className="flex gap-4">
              ...
              <div className="w-48">
                ...
                <select
                  ...
                  className="w-full px-3 py-2.5 rounded-xl bg-gray-800 border border-gray-700 text-white text-sm font-bold focus:outline-none focus:border-violet-500/50 cursor-pointer"
                >
                  {TONES.map((t) => (
            ...
            {generatedContent && (
              <div className="space-y-4">
                ...
                <textarea
                  ...
                  rows={12}
                  className="w-full px-4 py-3 rounded-xl bg-gray-950 border border-gray-700 focus:border-violet-500/50 focus:outline-none focus:ring-2 focus:ring-violet-500/20 text-gray-300 text-sm leading-relaxed resize-none font-sans transition-all"
                />
      ...
      <div className="min-h-[60vh]">
        <ContentReview key={refreshKey} workspaceId={currentWorkspace.id} />
      </div>
    </div>
  )
}
